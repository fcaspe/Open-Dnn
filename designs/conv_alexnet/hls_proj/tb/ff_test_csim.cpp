...
/*
  ...
  */
    ...
    ap_int<512>* output0_map = (ap_int<512>*)malloc(24200*sizeof(ap_int<512>));
...
void construct_para(ap_uint<32>* para)
{
	  /* Here we configure one layer execution only for accelerator 0 of sub_net_0 */
      /* Accelerator 1 of sub_net_0 is not activated, as the number of layer set (at para[512]) is 0*/
      int i;
	  //0-1.layer_num
	  para[0] = 2; /*number of layer tasks to be executed by this accelerator. Subsequent layer information can be added in para[32+...], para[48+...], and so on/*/
...
/*	  //0-1.conv para (First task)
    ...
    /*Pooling parameters*/
	  ...
	  para[256+0] = 1;
	  //1-2. Pool params
	  para[256+32+0] = 55; //R_in
	  para[256+32+1] = 55; //C_in
	  para[256+32+2] = 96; //N
	  para[256+32+3] = 2;  //K
	  para[256+32+4] = 27; //R
	  para[256+32+5] = 27; //C
	  ...
	  para[256+32+7] = 0;  //P
	  para[256+32+8] = 0;  //act
	  para[256+32+9] = 0;  //in_offset
	  para[256+32+10] = 0; //out_offset
	  para[256+32+11] = 0;
	  para[256+32+12] = 0;
	  para[256+32+13] = 0;
	  para[256+32+14] = 0;
	  para[256+32+15] = 1; //pool enable (will take data from output buffer and store in the output buffer too)
/*
	  //1-3.conv_para
	  para[256+32+0] = 1; //N
	  para[256+32+1] = 3; //K
