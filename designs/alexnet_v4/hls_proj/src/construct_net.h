...
#pragma HLS INTERFACE m_axi port=param_port offset=slave depth=1024 bundle=PARAM_IN
//#pragma HLS INTERFACE s_axilite port=bias_in bundle=CRTL_BUS
//#pragma HLS INTERFACE m_axi port=bias_in offset=slave depth=69144 bundle=BIAS_IN
#pragma HLS INTERFACE s_axilite port=weight_in bundle=CRTL_BUS
#pragma HLS INTERFACE m_axi port=weight_in offset=slave depth=69144 bundle=WEIGHT_IN 
...
#pragma HLS INTERFACE bram port=data_out_1        
	...
	if (select == 0)
	{
		...
		acc1_mem_outport_offset = 2028;
	}
	else
	{
		acc0_mem_inport_offset = 1352;
...
void sub_net_2(  
        Tparam param_port[1024],
        data_type_itf weight_in[1859880],
        
        data_type_itf data_in_0[4056],
        data_type_itf data_out_0[3380],
        data_type_itf data_in_1[3380],
        data_type_itf data_out_1[64],
        int select ) {
...
#pragma HLS INTERFACE m_axi port=param_port offset=slave depth=1024 bundle=PARAM_IN
//#pragma HLS INTERFACE s_axilite port=bias_in bundle=CRTL_BUS
//#pragma HLS INTERFACE m_axi port=bias_in offset=slave depth=1859880 bundle=BIAS_IN
#pragma HLS INTERFACE s_axilite port=weight_in bundle=CRTL_BUS
#pragma HLS INTERFACE m_axi port=weight_in offset=slave depth=1859880 bundle=WEIGHT_IN 
...
#pragma HLS INTERFACE bram port=data_out_1        
	...
	if (select == 0)
	{
		acc0_mem_inport_offset = 0;
		acc0_mem_outport_offset = 0;
		acc1_mem_inport_offset = 1690;
		acc1_mem_outport_offset = 32;
	}
	else
	{
		acc0_mem_inport_offset = 2028;
		acc0_mem_outport_offset = 1690;
		acc1_mem_inport_offset = 0;
		acc1_mem_outport_offset = 0;
	}
